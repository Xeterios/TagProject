...
public class CreateMap implements Cmd {
    ...
    public void Execute(CommandSender sender, Main main, Config config) {
        if (args.length > 1) {
            if (config.GetMap(args[1].toLowerCase()) == null){
                ...
                config.SaveConfig();
                sender.sendMessage(config.getPluginPrefix() + ChatColor.translateAlternateColorCodes('&', config.getPluginColor()) + "New map added: " + args[1]);
            } else {
    ...
    public void SendFaultyMessage(CommandSender sender, Main main, Config config) {
        if (args.length == 1){
            ...
            sender.sendMessage(config.getPluginPrefix() + ChatColor.RED + "This map already exists.");
        } else {
