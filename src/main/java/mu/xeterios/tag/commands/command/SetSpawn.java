        ...
        if (sender instanceof Player){
            if (args.length > 1){
                ...
                if (config.GetMap(args[1].toLowerCase()) != null) {
                    ...
                    config.SaveConfig();
                    sender.sendMessage(config.getPluginPrefix() + ChatColor.translateAlternateColorCodes('&', config.getPluginColor()) + "New spawnpoint set: " + map.getSpawn().getWorld().getName() + ": " + (int) map.getSpawn().getX() + " " + (int) map.getSpawn().getY() + " " + (int) map.getSpawn().getZ());
                } else {
